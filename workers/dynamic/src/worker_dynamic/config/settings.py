"""Environment-backed settings for the dynamic worker bootstrap.

The module exposes :class:`WorkerSettings`, an immutable configuration object
materialized from process environment variables via :meth:`WorkerSettings.from_env`.
Defaults in this module are effective runtime defaults used when variables are
absent.
...
- ``WORKER_PROFILE`` (default: ``"DYNAMIC_BASELINE"``)
...
- ``DYNAMIC_AGENT_PATH`` (default: ``"/opt/agent-dynamic-c/agent_dynamic"``)
- ``DYNAMIC_AGENT_TIMEOUT_MS`` (default: ``"5000"``, coerced to ``int``)
- ``APP_RUNTIME_MODE`` (default: ``"dev"``)

Bootstrap failures are observable as ``ValueError`` when any integer-backed
variable contains a non-integer value.
"""
...
class WorkerSettings:
    """Immutable runtime settings consumed by dynamic worker components.

    Instances are intended to be created once during process bootstrap and then
    injected into services. No normalization beyond integer coercion is applied;
    string values are passed through exactly as resolved from environment/defaults.
    """
    ...
    def from_env(cls) -> "WorkerSettings":
        """Build worker settings from environment with deterministic defaults.

        Args:
            cls: Class used to construct the immutable settings object.

        Returns:
            WorkerSettings: Fully populated configuration snapshot for the current
            process.

        Raises:
            ValueError: If ``DB_PORT``, ``WORKER_POLL_SECONDS``,
            ``APP_CORRELATION_OBSERVED_RUNS``, or ``DYNAMIC_AGENT_TIMEOUT_MS``
            is not a valid base-10 integer.

        Side Effects:
            Reads process environment variables through ``os.getenv``.
        """
        settings = cls(
            ...
            worker_profile=os.getenv("WORKER_PROFILE", "DYNAMIC_BASELINE"),
            ...
            audit_system_user_email=os.getenv("APP_AUDIT_SYSTEM_USER_EMAIL", "system@ppaob.local"),
            agent_binary_path=os.getenv("DYNAMIC_AGENT_PATH", "/opt/agent-dynamic-c/agent_dynamic"),
            agent_timeout_ms=int(os.getenv("DYNAMIC_AGENT_TIMEOUT_MS", "5000")),
